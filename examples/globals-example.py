...
from aviary.cgcnn.globals.data import GlobalsData, collate_globals
from aviary.cgcnn.globals.model import GlobalsNet
...
def main(
    ...
    **kwargs,
    ...
    dataset = GlobalsData(
        df=df, elem_emb=elem_emb, task_dict=task_dict, glob_fea=glob_fea, **dist_dict
    )
...
def input_parser():
    ...
    parser.add_argument(
        "--globals-path",
        type=str,
        default="data/datasets/tests/global_22.dat",
        help="Path to list of global feature names.",
    )
    ...
    if os.path.isfile(args.globals_path):
        with open(args.globals_path, "r") as f:
            args.glob_fea = f.read().splitlines()
    else:
        raise ValueError("No global features specified.")
